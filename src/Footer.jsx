...
export default function Footer() {
  ...
  const { address, brandName, email, facebookUrl, favIcon, footerAboutText, instagramUrl, linkedinUrl, logo, twitterUrl, workingHours, youtubeUrl
  } = settings || {}
        ...
        <div>
          {
            settings ?
              ...
              :
              <div
                className={`flex items-center gap-2.5 font-sans font-black text-2xl tracking-tight cursor-pointer select-none transition-transform duration-300 active:scale-95 ${isScrolled ? 'text-blue-600' : 'text-white'
                  }`}
                onClick={() => { navigate('/'); }}
              >
        ...
        <div >
          ...
          <div className='flex  gap-2 flex-col'>
            ...
            <div className='text-green-400 text-sm'>
              {
                settings ? <p>{workingHours}</p>
                  :
                  <p>Open : Monday to Friday from 9 AM to 10 PM</p>
              }
            </div>
          </div>
        </div>
